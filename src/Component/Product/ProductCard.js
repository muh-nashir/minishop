...
const ProductCard = (props) => {
    return (
        <div className="border border-solid border-slate-400 p-4 rounded-md text-center">
            ...
            <Link to={`/products/${props.product.id}`} className="px-6 py-3 bg-green-500 hover:bg-green-400 border-zero text-white hover:text-black font-bold rounded-md mt-8 block">Go to Product</Link>
        </div>
    )
}
